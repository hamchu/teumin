            ...
            while (state) {
                ...
                switch (data.getDataType()) // case 쌓이면 가독성 내려감. 나중에 구조 적절히 수정함.
                {
                    ...
                    case FOOD_TRUCK_INQUIRY_REQUEST: { // test해볼려고 일단 대충 박아둠.. sorry..
                        String sqlstr = "select * from food_truck where category='" + (String)data.getObject(0) + "'";
                        ResultSet resultSet = statement.executeQuery(sqlstr);
                        ...
                        while(resultSet.next())
                        {
                            ...
                            data.addObject(null_point);
                            data.addObject(resultSet.getString("food_truck_name"));
                            network.write(data);
                        }
                        resultSet.close();
                        data = new Data(DataType.FOOD_TRUCK_INQUIRY_RESPOND);
